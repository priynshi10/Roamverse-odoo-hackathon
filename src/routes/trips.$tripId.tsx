...
export const Route = createFileRoute("/trips/$tripId")({
  head: () => ({ meta: [{ title: "Trip — RoamVerse" }] }),
                          ...
                          <div key={a.id} className="group flex items-start gap-3 p-3 rounded-xl bg-secondary/40 hover:bg-secondary transition-colors">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-2 flex-wrap">
                                ...
                                <div className="text-sm font-semibold text-foreground/80">₹{a.cost.toLocaleString("en-IN")}</div>
                              </div>
  ...
  return (
    <div className="p-3 rounded-xl border border-border space-y-2 bg-card">
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        ...
        <Input type="number" placeholder="Cost (INR)" value={cost || ""} onChange={(e) => setCost(Number(e.target.value))} />
      </div>
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-5 border-border/60 shadow-soft">
          ...
          <div className="font-display text-3xl font-bold mt-1 text-gradient">₹{total.toLocaleString("en-IN")}</div>
          {overBudget && <p className="text-xs text-destructive mt-2">⚠ Over recommended ₹{maxBudget.toLocaleString("en-IN")}</p>}
        </Card>
        <Card className="p-5 border-border/60 shadow-soft">
          <div className="text-sm text-muted-foreground">Daily average</div>
          <div className="font-display text-3xl font-bold mt-1">₹{Math.round(dailyAvg).toLocaleString("en-IN")}</div>
