...
function DashboardPage() {
  ...
  const stats = [
    ...
    { label: "Total budget", value: `$${totalSpend.toLocaleString()}`, icon: Wallet, color: "from-emerald-500 to-teal-500" },
            ...
            <div className="h-64">
              <ResponsiveContainer>
                <PieChart>
                  ...
                  <Tooltip formatter={(v: any) => `$${v.toLocaleString()}`} contentStyle={{ background: "var(--popover)", border: "1px solid var(--border)", borderRadius: 12 }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          ) : <EmptyChart />}
